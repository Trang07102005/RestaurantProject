...
const WaiterOrders = () => {
  ...
  const fetchTables = async () => {
    try {
      const [tablesRes, reservationsRes] = await Promise.all([
        axios.get("http://localhost:2095/api/tables"),
        axios.get("http://localhost:2095/api/reservations"),
      ]);

      // Thêm thông tin reservation vào table
      const tablesWithReservations = tablesRes.data.map((table) => {
        const tableReservations = reservationsRes.data.filter(
          (r) => r.table?._id === table._id && r.status !== "cancelled"
        );
        
        // Tìm reservation sắp tới nhất (chỉ pending)
        const upcomingReservation = tableReservations
          .filter((r) => r.status === "pending" && new Date(r.reservationTime) > new Date())
          .sort((a, b) => new Date(a.reservationTime) - new Date(b.reservationTime))[0];

        // Tính toán trạng thái hiển thị động
        let displayStatus = table.status;
        let hasUpcomingWindow = false;
        if (upcomingReservation && table.status === "available") {
          const now = new Date();
          const resTime = new Date(upcomingReservation.reservationTime);
          const diffMin = (resTime.getTime() - now.getTime()) / (1000 * 60);
          // Nếu còn <= 30 phút tới giờ đặt, hiển thị như "Đã đặt trước"
  ...
  const handleSelectTable = async (table) => {
    ...
    const isReservedSoon = table.displayStatus === "reserved";
    if (isReservedSoon) setShowReservedModal(true);
  };
          ...
          {tables.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 gap-5">
              {tables.map((table) => {
                ...
                const effectiveStatusKey = table.displayStatus || table.status;
                const status =
                  statusStyles[effectiveStatusKey] || statusStyles.available;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 text-sm text-gray-600 space-y-1">
                      ...
                      {table.upcomingReservation && table.hasUpcomingWindow && (
                        <p className="flex items-center gap-1 text-xs text-orange-600 font-medium">
                          🕐 Đặt lúc{" "}
                          {new Date(
                            table.upcomingReservation.reservationTime
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg animate-fade-in mt-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              ...
              title="Tạo order mới cho bàn này"
        ...
        <ReservedTableModal
          ...
          onArrived={async () => {
            try {
              ...
              alert("Lỗi xác nhận khách đến: " + (err.response?.data?.message || err.message));
            }
          }}
          onCancel={async () => {
            try {
              // Tìm reservation pending/confirmed của bàn này
              const reservations = await axios.get(
                `http://localhost:2095/api/reservations/byTable/${selectedTable._id}`
              );
              ...
              if (activeReservation) {
                // Hủy reservation qua API
                await axios.delete(
                  `http://localhost:2095/api/reservations/${activeReservation._id}`
                );
              }
